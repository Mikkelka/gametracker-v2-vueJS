...
export function useDragAndDrop() {
  ...
  async function handleDrop(event) {
    ...
    if (game.status === newStatus && dropTarget.classList.contains('card')) {
      ...
      // Omorganisér rækkefølgen inden for samme liste
      const gamesInSameList = gameStore.games.filter(g => g.status === newStatus);
      const rect = dropTarget.getBoundingClientRect();
      const midY = rect.top + rect.height / 2;
      ...
      // Opdater rækkefølge for det flyttede spil
      await gameStore.updateGameOrder([
        { id: game.id, order: newOrder, status: newStatus }
      ]);
      
      // Fixing for numerisk orden (1, 2, 3...)
      const updatedGames = gameStore.games
